      ...
      {contact.status === 'pending-outgoing' ? (
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {messages.map((m) => (
              <div key={m.id} style={{ display: 'flex', justifyContent: m.direction === 'sent' ? 'flex-end' : 'flex-start' }}>
                <div
                  style={{
                    maxWidth: '70%',
                    padding: '8px 12px',
                    borderRadius: 12,
                    background: m.direction === 'sent' ? (m.status === 'sent' ? '#8fb3f7' : '#2f6fed') : '#eee',
                    color: m.direction === 'sent' ? 'white' : 'black',
                  }}
                >
